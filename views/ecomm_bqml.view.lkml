    ...
    sql: (SELECT * except(user_id1) FROM
              (SELECT user_id
                ...
                FROM (
                  ...
                  FROM `mazzero-looker.thelook.order_items` as oi
                  JOIN `mazzero-looker.thelook.inventory_items` as inv ON inv.id = oi.inventory_item_id
      ...
      JOIN (SELECT user_id as user_id1
            ...
            FROM `mazzero-looker.thelook.events`
...
view: ecomm_predict {
  derived_table: {
    sql: SELECT
      ...
      FROM
        ML.PREDICT(MODEL `mazzero-looker.thelook.kmeans_model5`,
        (SELECT * FROM
